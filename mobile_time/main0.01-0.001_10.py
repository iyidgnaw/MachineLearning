...
def train(user_cart,time_cart,u ,x ,Tt, time_interval):
	...
	for i in xrange(len(user_cart)-1):
		...
		hl = h
		dhlist.append(np.dot(-(1-sigmoid(Xij))*(item-neg_item),Mi))#save the dh for each bpr step
...
#layer1
	...
	for i in range(len(user_cart)-1)[::-1]:
		...
		time_interval[interval_typenow,:] += -learning_rate*(dtime_interval.reshape(time_size,)+lamda*time_interval[interval_typenow,:])
...
def predict(all_cart,allresult):
	...
	for n in xrange(len(all_cart)):
		...
		for j in xrange(i,len(user_cart)-1):
			...
			predict_matrix = np.dot(h, np.dot(x,Mnext).T)
...
while True:
	...
	for i in xrange(len(all_cart)):
		...
		for behavior in behavior_list:
			user_cart.append(behavior[0])
			time_cart.append(behavior[1])
		u,x,loss=train(user_cart, time_cart, u,x,Tt,time_interval)
		sumloss+=loss
		if i%100==0:
			print i
