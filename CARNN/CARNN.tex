...
We can introduce various aggregation operations to model different interactions among multiple input contexts. In this work, we study three typical aggregation operations as follows.

\begin{itemize}
\item \textbf{Linear Method}: Linear computation is the most common method to calculate the aggregation of all contextual operating matrices, It treats the effects of each context separately, and the $\textbf{M}_{c_{I,k}^{u}}$ can be calculated as :
\begin{equation} \label{eqMadd}
\textbf{M}_{c_{I,k}^{u}} = \sum_{c_{i,k}^{u}\in C^I} \textbf{M}_{c_{i,k}^{u}} ~.
\end{equation}

\item \textbf{Multiplied Method}: In STRNN  \cite{liu2016strnn} ,the author incorporates temporal and spatial contexts into conventional RNN by mutipling time-specific operating matrix and distance-specific operating matrix. Analogously, we can treat input contextual operating matrices in the same way :
\begin{equation}
\textbf{M}_{c_{I,k}^{u}} = \prod_{c_{i,k}^{u}\in C^I} \textbf{M}_{c_{i,k}^{u}} ~.
\end{equation}

\item \textbf{Combination-specific Method}: Other than handling these input contexts matrices, the $\textbf{M}_{c_{I,k}^{u}}$ can also be a combination-specific matrix. It means that the $\textbf{M}_{c_{I,k}^{u}}$ is equal to a matrix representing a combination for some specific input contexts at time $k$. For example, if there are two input contexts: weather and mood, then each combination has its own matrix as shown in Table \ref{combination}. In that case, when the weather is sunny and the mood is happy, the  $\textbf{M}_{c_{I,k}^{u}}$ will be $\textbf{M}_{11}$. 
...
\begin{table*}[htbp]
  ...
  \label{tab:result}%
\end{table*}%
%2 figure for each dataset


\subsection{Impact of Parameters}
%1 table 
%1 figure of the performance comparison for RNN and CA-RNN in different length of hidden layer
...
% $\textbf{M}_{c_{I,k+1}^{u}}$


% $\textbf{M}_{c_{I,k+2}^{u}}$


% $\textbf{W}_{\left\lfloor t_k^u-t_{k-1}^u \right\rfloor }$


% $\textbf{W}_{\left\lfloor t_{k+1}^u-t_{k}^u \right\rfloor }$


% $\textbf{W}_{\left\lfloor t_{k+2}^u-t_{k+1}^u \right\rfloor }$
